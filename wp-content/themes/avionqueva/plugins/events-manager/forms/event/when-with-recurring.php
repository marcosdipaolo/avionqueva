...
?>
	...
	<div id="em-form-with-recurrence" class="event-form-with-recurrence event-form-when">
		<p>
			<input class="uk-checkbox" type="checkbox" id="em-recurrence-checkbox" name="recurring" value="1" <?php if($EM_Event->is_recurring()) echo 'checked' ?> />
			<label for="em-recurrence-checkbox">
				<?php _e('This is a recurring event.', 'events-manager'); ?>
			</label>
		</p>
		<!-- WHEN-WITH-RECURRING.PHP -->
		<p class="em-date-range">
			<span class="em-recurring-text" style="display:block">
				<?php _e ( 'Recurrences span from ', 'events-manager'); ?>
			</span>
			<span class="em-event-text">
		...
		<p id="after-date">
			<span class="em-recurring-text">
				<?php _e('Events start from','events-manager'); ?>
			</span>
			<span class="em-event-text">
				<?php _e('Event starts at','events-manager'); ?>
			</span>
			...
			<input class="uk-checkbox" type="checkbox" class="em-time-allday" name="event_all_day" id="em-time-all-day" value="1" <?php if(!empty($EM_Event->event_all_day)) echo 'checked="checked"'; ?> />
			<label for="em-time-all-day"><?php _e('All day','events-manager'); ?></label>
		</p>
		<div class="em-recurring-text">
			<p>

				<div class="uk-margin">
					<select class="uk-select" id="recurrence-frequency" name="recurrence_freq">
						<?php
				...
				?>
					...
					<label for="recurrence-frequency">
				...
				<span class='interval-desc' id="interval-daily-singular">
					<?php _e ( 'day', 'events-manager')?>
				</span>
				<span class='interval-desc' id="interval-daily-plural">
					<?php _e ( 'days', 'events-manager') ?>
				</span>
				<span class='interval-desc' id="interval-weekly-singular">
					<?php _e ( 'week on', 'events-manager'); ?>
				</span>
				<span class='interval-desc' id="interval-weekly-plural">
					<?php _e ( 'weeks on', 'events-manager'); ?>
				</span>
				<span class='interval-desc' id="interval-monthly-singular">
					<?php _e ( 'month on the', 'events-manager')?>
				</span>
				<span class='interval-desc' id="interval-monthly-plural">
					<?php _e ( 'months on the', 'events-manager')?>
				</span>
				<span class='interval-desc' id="interval-yearly-singular">
					<?php _e ( 'year', 'events-manager')?>
				</span>
				<span class='interval-desc' id="interval-yearly-plural">
					<?php _e ( 'years', 'events-manager') ?>
				</span>
				</div>
			...
			<p class="em-duration-range">
				<?php echo sprintf(__('Cada evento abarca %s día(s)','events-manager'), '<input id="end-days" type="text" size="8" maxlength="8" name="recurrence_days" value="'. $EM_Event->recurrence_days .'" />'); ?>
			</p>
			<p class="em-range-description">
				<em>
					<?php _e( 'For a recurring event, a one day event will be created on each recurring date within this date range.', 'events-manager'); ?>
				</em>
			</p>
		</div>
