...
<form enctype='multipart/form-data' id="event-form" class="em-event-admin-editor <?php if( $EM_Event->is_recurring() ) echo 'em-event-admin-recurring' ?>" method="post" action="<?php echo esc_url(add_query_arg(array('success'=>null))); ?>">
	<div class="wrap">
		...
		<em><span style="font-size: 14px;font-family: sans-serif;font-weight: 700;">Los eventos aquí publicados deben estar relacionados con el campo de la Literatura Infantil y Juvenil. Es necesario que incluyan una imagen que tenga un ancho de alrededor de los 1920px. Deberán ser imágenes, no flyers. Toda la información debe ir incluida como texto, en los campos de carga. ¡Muchas gracias por subir a Avión que Va!  </span></em>
	...
	<p class="submit">
	    ...
	    <button class="uk-button uk-button-danger" type="submit"><?php echo esc_attr(sprintf( __('Submit %s','events-manager'), __('Event','events-manager') )); ?></button>
	    <?php else: ?>
	   <button class="uk-button uk-button-danger" type="submit"><?php echo esc_attr(sprintf( __('Update %s','events-manager'), __('Event','events-manager') )); ?></button>
	    <?php endif; ?>
	</p>
